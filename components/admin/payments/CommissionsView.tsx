...
const CommissionsView: React.FC = () => {
    ...
    const fetchPayments = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${API_URL}/admin/calendar-payments?type=comissoes&month=${selectedMonth}&year=${selectedYear}`);
            ...
            data = data.map(payment => {
                ...
                // For commissions, the primary recipient is the consultant
                const consultantId = payment.consultor_id || contract?.consultor_id;
                const clientId = contract?.user_id;

                const client = clientsData?.find(c => c.id === clientId);
                const consultant = consultantsData?.find(c => c.id === consultantId);
                ...
                return {
                    ...
                    consultor_name: consultant ? (consultant.nome_fantasia || consultant.nome) : 'Desconhecido'
                };
            });

            setPayments(data);
        } catch (err) {
            console.error('[CommissionsView] Error fetching payments:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            console.error('[CommissionsView] Error confirming payment:', err);
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-[#002B49]">Comissões</h1>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={7} className="p-8 text-center text-slate-500">Nenhuma comissão encontrada para este mês.</td></tr>
                            ) : (
