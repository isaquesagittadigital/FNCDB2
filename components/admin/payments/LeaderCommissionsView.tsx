...
const LeaderCommissionsView: React.FC = () => {
    ...
    const fetchPayments = async () => {
        setLoading(true);
        try {
            const res = await fetch(`${API_URL}/admin/calendar-payments?type=comissoes_lider&month=${selectedMonth}&year=${selectedYear}`);
            ...
            const { data: contractsData } = await supabase.from('contratos').select('id, codigo, user_id, consultor_id, lider_id');
            ...
            data = data.map(payment => {
                ...
                // For Leader commissions, the recipient is the leader_id in contract
                const leaderId = contract?.lider_id;
                const clientId = contract?.user_id;

                const client = clientsData?.find(c => c.id === clientId);
                const leader = consultantsData?.find(c => c.id === leaderId);
                ...
                return {
                    ...
                    consultor_name: leader ? (leader.nome_fantasia || leader.nome) : 'Desconhecido'
                };
            });

            setPayments(data);
        } catch (err) {
            console.error('[LeaderCommissionsView] Error fetching payments:', err);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleConfirmPayment = async () => {
        ...
        try {
            ...
            console.error('[LeaderCommissionsView] Error confirming payment:', err);
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-[#002B49]">Comissões de Líder</h1>
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-100 text-slate-500 text-sm">
                                <th className="p-4 font-medium">Líder (Consultor)</th>
                                <th className="p-4 font-medium text-center">Data</th>
                                <th className="p-4 font-medium text-center">Status</th>
                                <th className="p-4 font-medium text-center">Contrato</th>
                                <th className="p-4 font-medium text-center">Cliente do Contrato</th>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr><td colSpan={7} className="p-8 text-center text-slate-500">Nenhuma comissão de líder encontrada para este mês.</td></tr>
                            ) : (
                                payments.map((payment) => (
                                    <tr key={payment.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                                        <td className="p-4">
                                            <p className="font-medium text-[#002B49]">{payment.consultor_name}</p>
                                        </td>
                                        ...
                                        <td className="p-4 text-center text-slate-600">{payment.client_name}</td>
            ...
            <ConfirmPaymentModal
                ...
                paymentInfo={selectedPayment ? {
                    name: selectedPayment.consultor_name || 'Desconhecido',
...
export default LeaderCommissionsView;
