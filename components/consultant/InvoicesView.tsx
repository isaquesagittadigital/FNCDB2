...
const InvoicesView: React.FC<InvoicesViewProps> = ({ userProfile }) => {
  ...
  const fetchInvoicesAndCommissions = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/admin/calendar-payments?consultor_id=${userProfile.id}&year=${selectedYear}`);
      ...
      if (res.ok) {
        ...
        comData.forEach((item: any) => {
          if (item.data && typeof item.valor !== 'undefined') {
            const dateStr = item.data.substring(0, 10); // YYYY-MM-DD
            if (dateStr.startsWith(String(selectedYear))) {
              const month = parseInt(dateStr.split('-')[1], 10);
              // sum all values (dividendos, comissoes) exactly as CalendarView
              valuesByMonth[month] += Number(item.valor) || 0;
            }
          }
        });
      } else {
        console.error('Error fetching calendar payments');
      }
