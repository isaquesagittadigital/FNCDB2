...
const AddressTab: React.FC<AddressTabProps> = ({ userProfile, onUpdate, saving }) => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <FormSection title="Endereço">
                ...
                <Field label="Endereço" value={formData.logradouro_end} onChange={(val) => updateForm('logradouro_end', val)} required className="md:col-span-1" />
                <Field label="Número" value={formData.numero_end} onChange={(val) => updateForm('numero_end', val)} required className="md:col-span-1" />
                <Field label="Complemento" value={formData.complemento_end} onChange={(val) => updateForm('complemento_end', val)} placeholder="Opcional" className="md:col-span-1" />
                <Field label="Bairro" value={formData.bairro} onChange={(val) => updateForm('bairro', val)} required className="md:col-span-1" />

                <Field label="Cidade" value={formData.cidade} onChange={(val) => updateForm('cidade', val)} required className="md:col-span-1" />
                <div className="space-y-2 md:col-span-1">
                    ...
                    <input
                        ...
                        maxLength={2}
                        className="w-full px-4 py-3.5 bg-white border border-slate-200 rounded-xl text-sm text-[#002B49] font-medium uppercase focus:outline-none focus:ring-2 focus:ring-[#00A3B1]/10 focus:border-[#00A3B1]"
                    />
                </div>
            </FormSection>
